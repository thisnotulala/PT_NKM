...
{
    /**
     * Helper: hanya site manager
     */
    private function onlySiteManager()
    {
    ...
    /**
     * LIST JADWAL
     * Site Manager, Administrasi, Kepala Lapangan
     */
    ...
    public function store(Request $request)
    {
        ...
        // validasi fase milik proyek
        if ($phase->project_id != $project->id) {
            return back()->withInput()
                ->withErrors(['project_phase_id' => 'Tahapan tidak sesuai proyek.']);
        }

        // hitung tanggal selesai
        $tanggalSelesai = date(
            'Y-m-d',
            strtotime($data['tanggal_mulai'] . ' + ' . ($data['durasi_hari'] - 1) . ' days')
        );

        // validasi rentang proyek
        if (
            $data['tanggal_mulai'] < $project->tanggal_mulai ||
            $tanggalSelesai > $project->tanggal_selesai
        ) {
            return back()->withInput()->withErrors([
                'tanggal_mulai' =>
                    "Jadwal harus di dalam rentang proyek ({$project->tanggal_mulai} s/d {$project->tanggal_selesai})."
            ]);
        }
        ...
        if ($exists) {
            return back()->withInput()
                ->withErrors(['project_phase_id' => 'Jadwal tahap ini sudah ada.']);
        }
        ...
        if ($overlap) {
            return back()->withInput()->withErrors([
                'tanggal_mulai' => 'Tanggal tahap bentrok dengan jadwal tahap lain.'
            ]);
        }
        ...
        return redirect()->route('jadwal.index')
            ->with('success', 'Jadwal tahap berhasil dibuat.');
    }
    ...
    public function update(Request $request, ProjectPhaseSchedule $schedule)
    {
        ...
        // 1 fase 1 jadwal (kecuali dirinya)
        $exists = ProjectPhaseSchedule::where('project_id', $project->id)
            ->where('project_phase_id', $phase->id)
        ...
        return redirect()->route('jadwal.index')
            ->with('success', 'Jadwal tahap berhasil diupdate.');
    }
    ...
    public function destroy(ProjectPhaseSchedule $schedule)
    {
        ...
        return redirect()->route('jadwal.index')
            ->with('success', 'Jadwal tahap berhasil dihapus.');
    }
}
